...
def make_id(prefix: str, author: str, en: str, ko: str, note: str) -> str:
  # 고정 세트이지만 "조합(영문/한글/노트)"이 다르면 서로 다른 id가 되도록 구성
  h = hashlib.sha1(f"{prefix}|{author}|{en}|{ko}|{note}".encode("utf-8")).hexdigest()[:10]
  return f"{prefix}-{h}"

def gen_block(prefix, authors, templ_en, templ_ko, notes, n, max_tries=200000):
  """
  n개가 될 때까지 (author, en, ko, note) 조합을 랜덤으로 뽑아 중복 없이 채움.
  고정 seed를 쓰면 결과는 항상 동일(=고정 세트).
  """
  seen = set()
  uniq = []
  tries = 0
  ...
  while len(uniq) < n and tries < max_tries:
    ...
    qid = make_id(prefix, author, en, ko, note)
    if qid in seen:
      continue
  ...
  if len(uniq) < n:
    raise RuntimeError(
      f"Could not generate {n} unique items (got {len(uniq)}). "
      f"Increase template variety or lower n."
    )
...
if __name__ == "__main__":
  # ✅ 고정 100세트 (원하면 숫자만 바꾸면 됨)
  random.seed(20260215)

  N = 100
  morning = gen_block("AM", MORNING_AUTHORS, MORNING_TEMPLATES_EN, MORNING_TEMPLATES_KO, MORNING_NOTES, N)
  afternoon = gen_block("PM", AFTERNOON_AUTHORS, AFTERNOON_TEMPLATES_EN, AFTERNOON_TEMPLATES_KO, AFTERNOON_NOTES, N)

  dump_jsonl("quotes_morning.jsonl", morning)
  dump_jsonl("quotes_afternoon.jsonl", afternoon)

  print(f"Generated {N}/{N} quotes (fixed set).")
